...
public class LineChartView extends View {
    ...
    public void workWith(List<String> keys, List<Double> values) {
        if (keys != null && keys.size() != 0 && values != null && values.size() != 0) {
            ...
            COUNT = Math.min(keys.size(), values.size()); // 数据点数量
    ...
    public void work() {
        post(new Runnable() {
            ...
            public void run() {
                ...
                final int averageWidth = WIDTH / COUNT; // 每个key可绘制的宽度

                // 循环遍历，设置每个key的绘制起点
                for (int i = 0; i < COUNT; i++) {
                    KeyView keyView = keyViewList.get(i);
                    keyView.setTextCenter(averageWidth / 2 + i * averageWidth, HEIGHT - DensityUtil.dp2px(getContext(), 4));
                }
                ...
                verticalDashesLine.setPoints(valueView.centerX,
                        valueView.centerY,
                        valueView.centerX,
                        HEIGHT - DensityUtil.dp2px(getContext(), 20));

                floatValueView = new FloatValueView(getContext(), WIDTH, HEIGHT);
